package bEssentials.Configuração;
...
public class MyConfigManager {
	...
	public MyConfig getNewConfig(String fileName, String[] header) {
		...
		MyConfig config = new MyConfig(getConfigContent(fileName), file,
	...
	private File getConfigFile(String file) {
		...
		if (file.contains("/")) {
			if (file.startsWith("/")) {
				configFile = new File(this.plugin.getDataFolder()
						+ file.replace("/", File.separator));
			} else {
				configFile = new File(this.plugin.getDataFolder()
						+ File.separator + file.replace("/", File.separator));
			}
		} else {
		...
		try {
			...
			while ((currentLine = reader.readLine()) != null) {
				if (currentLine.startsWith("#")) {
					String addLine = currentLine.replaceFirst("#", pluginName
